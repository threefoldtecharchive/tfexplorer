...
func (f ReservationFilter) WithNodeID(id string) ReservationFilter {
	...
	for _, typ := range []string{"containers", "volumes", "zdbs", "kubernetes", "proxies", "reserve_proxies", "subdomains", "domain_delegates", "gateway4to6", "debug"} {
...
func (r *Reservation) Validate() error {
	...
	for _, w := range r.DataReservation.Debug {
		workloaders = append(workloaders, w)
	}
...
func (r *Reservation) Workloads(nodeID string) []Workload {
	...
	for _, wl := range data.Debug {
		if len(nodeID) > 0 && wl.NodeId != nodeID {
			continue
		}
		wrkl := newWrkl(
			fmt.Sprintf("%d-%d", r.ID, wl.WorkloadId),
			generated.WorkloadTypeDebug,
			wl.NodeId)
		wrkl.Content = wl
		workloads = append(workloads, wrkl)
	}
