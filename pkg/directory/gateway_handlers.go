...
func (s *GatewayAPI) registerGateway(r *http.Request) (interface{}, mw.Response) {
	...
	if _, err := s.Add(r.Context(), db, gw); err != nil {
		return nil, mw.GenericDBError(mw.DBError{Cause: err, Message: errFailedToRegisterGateway.Error()})
	}
...
func (s *GatewayAPI) gatewayDetail(r *http.Request) (interface{}, mw.Response) {
	...
	if err != nil {
		return nil, mw.GenericDBError(mw.DBError{Cause: err, Message: fmt.Sprintf("gateway with id %s not found", gatewayID)})
	}
...
func (s *GatewayAPI) listGateways(r *http.Request) (interface{}, mw.Response) {
	...
	if err != nil {
		return nil, mw.GenericDBError(mw.DBError{Cause: err, Message: errFailedToListGateways.Error()})
	}
...
func (s *GatewayAPI) updateUptimeHandler(r *http.Request) (interface{}, mw.Response) {
	...
	if err := s.updateUptime(r.Context(), db, gatewayID, int64(input.Uptime)); err != nil {
		return nil, mw.GenericDBError(mw.DBError{Cause: err, Message: fmt.Sprintf("failed to update gateway with gatewayID %s", gatewayID)})
	}
...
func (s *GatewayAPI) updateReservedResources(r *http.Request) (interface{}, mw.Response) {
	...
	if err := s.updateReservedCapacity(r.Context(), db, gatewayID, input.ResourceAmount); err != nil {
		return nil, mw.GenericDBError(mw.DBError{Cause: err, Message: fmt.Sprintf("failed to update gateway reserved capacity with gatewayID %s", gatewayID)})
	}
	if err := s.updateWorkloadsAmount(r.Context(), db, gatewayID, input.WorkloadAmount); err != nil {
		return nil, mw.GenericDBError(mw.DBError{Cause: err, Message: fmt.Sprintf("failed to update gateway workloads amount with gatewayID %s", gatewayID)})
	}
